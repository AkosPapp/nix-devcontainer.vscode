...
function getFileFromTemplate(context, ...paths) {
...
async function updateNixProfile(context, flake) {
    ...
    await vscode.window.withProgress({
        ...
        return new Promise((resolve) => {
            ...
            const shellExecution = new vscode.ShellExecution('bash', ['-c', `
                    set -euo pipefail
                    nix print-dev-env ${flake.path} | grep -v -F 'LINENO' | sudo tee ${profile_path} > /dev/null
                    `]);
            const task = new vscode.Task(taskDefinition, vscode.TaskScope.Workspace, taskName, taskSource, shellExecution);
...
async function findFlakes(context) {
...
async function update_flakes(context) {
...
async function update_flake(context) {
...
async function find_and_update_all_flakes(context) {
